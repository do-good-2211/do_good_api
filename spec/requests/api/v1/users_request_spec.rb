...
RSpec.describe "User Request Spec" do
  ...
  describe "Get one user" do 
    ...
    it "can be have an array of good deeds" do 
      user = create(:user) 
      host = create(:user)
      good_deed_1 = create(:good_deed, host_id: host.id)
      good_deed_2 = create(:good_deed, host_id: host.id)
      user_good_deed_1 = create(:user_good_deed, user_id: user.id, good_deed_id: good_deed_1.id)
      user_good_deed_1 = create(:user_good_deed, user_id: user.id, good_deed_id: good_deed_2.id)

      expect(user.good_deeds).to eq([good_deed_1, good_deed_2])
      ...
      expect(user_parsed[:data][:attributes][:good_deeds].first[:name]).to eq(good_deed_1.name)
      expect(user_parsed[:data][:attributes][:good_deeds].second[:name]).to eq(good_deed_2.name)
    end
